...
- **Auth**: HTTP Basic Auth using the existing `SHIPSTATION_API_KEY` and
  `SHIPSTATION_API_SECRET` env vars (already present in `.env`, left over from this
  project's pre-V2-migration V1 client — see `__tests__/unit/shipstation/client.test.ts`
  for the prior Basic Auth pattern against `https://ssapi.shipstation.com`, which this
  reuses). Config resolution follows the same pattern as `getConfig()` in `client.ts` —
  throw a clear error if either var is missing.
...
No new env vars needed. `SHIPSTATION_API_KEY` and `SHIPSTATION_API_SECRET` already exist
in `.env` and are reused as-is for V1 Basic Auth, alongside the unrelated
`SHIPSTATION_V2_API_KEY` used by the existing V2 client.
...
- Verify a missing/invalid `SHIPSTATION_API_KEY`/`SHIPSTATION_API_SECRET` does not throw
  out of `createShippingLabelAction` (label creation still returns `status: "success"`).
